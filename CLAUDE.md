...
- **League naming in news prose** (manual `noticias-data.js` entries and the automated daily-news routine, per user instruction 2026-08-19): in any published text — title, excerpt, body, share captions — name a league only by its short form: `VPN`, `VPUG`, or `11x11`. Never append season suffixes like `T3`/`T6`/`Temporada 3` (those are fine as internal/code labels, e.g. comments in `seed_matches.js`, just not in reader-facing copy). Also, the league is the tournament/context, not a subject that "wins" or "loses" — never write "la VPN ganó"; the **club** is always the subject, the league is the frame: "Top Secret ganó esta noche en la VPN", "cayó 1-2 en la VPUG", "goleada por 4-0 en el 11x11".
